...
public class TrainMove {
	public void trainMoveSchedule(String codeTrain) {
		...
		if (codeTrain.equalsIgnoreCase("T2"))
		{
			for (int i = 0; i < so.getStation().size(); i++) {
				...
				m.addMove(s.getStationName(),s.getLat(),s.getLng(),s.getRanges(),s.getCodeStation());
...
//				Stations s = so.getStation().get(i);
//										
//				m2.addMove2(s.getStationName(),s.getLat(),s.getLng(),s.getRanges(),s.getCodeStation());
//				
...
//		}
	...
	public void trainMove2Schedule(String codeTrain) {
		StationsOptions so = new StationsOptions();
		DistanceOptions ds = new DistanceOptions();
		Moved2Options m2 = new Moved2Options();
		if(codeTrain.equalsIgnoreCase("T1")) {
			for (int i = so.getStation().size()-1; i >0; i--) {
				int stop = so.getStation().size() + 1;
				try {
					Thread.sleep(3000);
